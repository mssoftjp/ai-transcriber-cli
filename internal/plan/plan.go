...
func (p *Planner) Build(spec domain.JobSpec, input domain.InputInfo) (ExecutionPlan, error) {
	...
	plan := ExecutionPlan{
		...
		EstimatedIntermediateSize: estimateIntermediateSize(input),
	}
	plan.ChunkingMode = resolveChunkingMode(spec, plan)
	if plan.DiarizeCapable && input.DurationSec > 30 && plan.ChunkingMode == domain.ChunkingOff {
	...
	if plan.DiarizeCapable && input.DurationSec > 30 && plan.ChunkingMode == domain.ChunkingClient && !spec.AllowExperimentalDiarizeStitching {
	...
	if plan.ChunkingMode == domain.ChunkingClient && input.DurationSec > 0 {
		plan.Chunks = buildChunks(profile, input.DurationSec)
	}
	return plan, nil
}
