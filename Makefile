...
release-archive:
	@[ -n "$(VERSION)" ] || (echo "VERSION is required, e.g. make release-archive VERSION=v0.3.0 GOOS=darwin GOARCH=arm64" >&2; exit 1)
...
package:
	@[ -n "$(VERSION)" ] || (echo "VERSION is required, e.g. make package VERSION=v0.3.0" >&2; exit 1)
	mkdir -p "$(DIST_DIR)"
	GO="$(GO)" GOOS="$(GOOS)" GOARCH="$(GOARCH)" VERSION="$(VERSION)" COMMIT="$(COMMIT)" DATE="$(DATE)" \
		./scripts/package.sh --version "$(VERSION)" --output-dir "$(DIST_DIR)"
